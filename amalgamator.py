...
def amalgamate(dir_name,file_name):
    ...
    ic(df)
    ic(df.columns)
    xb_ranges_test =  [0,0.4,1.0]
    q2_ranges_test =  [1.0,5.0,12.0]
    t_ranges_test =  [0,1,6]
    phi_ranges_test =  [0,90,180,270,360]
...
if __name__ == "__main__":
    ...
    for index,dpath in enumerate(data_paths):
        print(dpath)
        dirname= '/mnt/d/GLOBUS/CLAS12/simulations/production/Fall_2018_Inbending/Counted/'+dpath
        ...
        for f in os.listdir(dirname):
            ...
            if os.path.isfile(f_path):
                #print(f)
                gen_type = "Norad" if "Norad/" in dpath else "Rad"
                sim_type = "Recon" if "Recon/" in dpath else "Gen"
                if sim_type == "Gen" and "recon" in f:
                    pass
                else:
                    col_name = '{}{}'.format(gen_type,sim_type)
                    if "all_generated_events" in f:
                        gen_num = f.split("_")[-2]
                        col_name = col_name + gen_num
                    print(col_name)
                    df = pd.read_pickle(f_path)
                    df = df.rename(columns={'counts':col_name})
                    df_segs.append(df[col_name])
                    
                #data['']
                #ic(df)
                #jobs_list.append(f)

    df_segs.append(df['ave_q'])
    df_segs.append(df['ave_x'])
    df_segs.append(df['ave_t'])
    df_segs.append(df['ave_p'])
    df_0 = pd.concat(df_segs,axis=1)
    ic(df_0)

    idx = df_0.columns.str.startswith('RadGen')
    df_0['RadGenTot'] = df_0.iloc[:,idx].sum(axis=1)
    ic(df_0)
    idx = df_0.columns.str.startswith('NoradGen')
